    ...
    const renderItem = ({ item }) => (
        <View style={[style.card, { padding: 15 }]}>
            <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 8 }}>
                ...
                />
                <View>
                    <Text style={{ color: theme.colors.onSurface, fontWeight: "bold" }}>{item.owner.name}</Text>
            ...
            <Text style={{ fontWeight: "bold", fontSize: 18, marginBottom: 8, color: theme.colors.onSurface }}>
                {item.title}
            </Text>
            <Text style={{ color: theme.colors.onSurface }}>Khu vực: {item.position || "Chưa cập nhật"}</Text>
            <Text style={{ color: theme.colors.onSurface }}>Diện tích: {item.area} m²</Text>
            <Text style={{ color: theme.colors.onSurface }}>Giới hạn: {item.limit_person} người</Text>
    ...
    return (
        <SafeAreaView style={style.container}>
            <FlatList
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity
                        onPress={() => navigation.navigate("RoomSeekingDetail", { postIntanceId: item.post.id })}
                    >
                        <View style={[style.card, { padding: 15 }]}>
                            <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 8 }}>
                                <Avatar.Image
