...
class ConversationSerializer(ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if user1.user_type == user2.user_type:
            raise serializers.ValidationError(
                f"CanNot create conversation with another {user1.user_type}."
            )

        if (
            Conversation.objects.filter(landlord=user1, tenant=user2).exists()
            or Conversation.objects.filter(landlord=user2, tenant=user1).exists()
        ):
            raise serializers.ValidationError(
                f"Conversation between {user1.id} and {user2.id} was exists."
            )
...
class MessageSerializer(ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        data["create_at"] = datetime.strftime(
            instance.created_date, "%Y-%m-%d %H:%M:%S"
        )
